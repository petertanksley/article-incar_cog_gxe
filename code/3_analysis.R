...
m0_base <- formula("cog_2cat_num ~ 
                   factor(sex) + factor(race_ethn) + scale(age) + scale(social_origins) + factor(stroke_ever) + factor(study) +
                   (1|hhidpn)")
m1_apoe <- formula("cog_2cat_num ~ factor(apoe_info99_4ct) +
                   factor(sex) + factor(race_ethn) + scale(age) + scale(social_origins) + factor(stroke_ever) + factor(study) +
                   (1|hhidpn)")
m2_incar <- formula("cog_2cat_num ~ factor(incar_ever) + 
                   factor(sex) + factor(race_ethn) + scale(age) + scale(social_origins) + factor(stroke_ever) + factor(study) +
                   (1|hhidpn)")
m3_main_eff <- formula("cog_2cat_num ~ factor(incar_ever) + factor(apoe_info99_4ct) +
                   factor(sex) + factor(race_ethn) + scale(age) + scale(social_origins) + factor(stroke_ever) + factor(study) +
                   (1|hhidpn)")
m4_int_eff <- formula("cog_2cat_num ~ factor(incar_ever)*factor(apoe_info99_4ct) +
                   factor(sex) + factor(race_ethn) + scale(age) + scale(social_origins) + factor(stroke_ever) + factor(study) +
                   (1|hhidpn)")

#run models 
m0 <- glmer(m0_base,      data=hrs, family=poisson(link="log"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
m1 <- glmer(m1_apoe,      data=hrs, family=poisson(link="log"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
m2 <- glmer(m2_incar,     data=hrs, family=poisson(link="log"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
m3 <- glmer(m3_main_eff,  data=hrs, family=poisson(link="log"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
m4 <- glmer(m4_int_eff,   data=hrs, family=poisson(link="log"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

export(c("m0","m1","m2","m3","m4"),
